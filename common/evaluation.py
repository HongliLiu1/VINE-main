...
class Evaluator:
    ...
    def classify_prediction(cls, pred_mask, batch):
        ...
        for _pred_mask, _gt_mask in zip(pred_mask, gt_mask):
            ...
            area_inter.append(_area_inter)  #0:219327,1:113
            area_pred.append(torch.histc(_pred_mask, bins=2, min=0, max=1)) # 239691,12576
            area_gt.append(torch.histc(_gt_mask, bins=2, min=0, max=1)) #231790,20477
        area_inter = torch.stack(area_inter).t()
        area_pred = torch.stack(area_pred).t()
        area_gt = torch.stack(area_gt).t()
        area_union = area_pred + area_gt - area_inter   #252154
        #  Union = Prediction Area + Ground Truth Area - Intersection
