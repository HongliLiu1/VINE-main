...
def proto_contrastive_loss(proto_q, proto_s, class_ids, margin=0.3, mode='positive_only', detach_query=True):
    """
    Compute contrastive loss between query and support prototypes.

    :param proto_q: [B, C] query prototype
    :param proto_s: [B, C] support prototype
    :param class_ids: [B] class IDs for pair labels
    :param margin: margin hyperparameter
    :param mode: 'positive_only' or 'full'
    :param detach_query: whether to detach query gradients
    :return: scalar contrastive loss
    """
    B = proto_q.size(0)
    if detach_query:
        proto_q = proto_q.detach()
    label_matrix = (class_ids.unsqueeze(1) == class_ids.unsqueeze(0)).float()  # [B, B]
    if mode == 'positive_only':
        mask = label_matrix
    else:
        mask = torch.ones_like(label_matrix)

    label_matrix = label_matrix * 2 - 1  # {1, -1}
    loss, count = 0.0, 0
    ...
    for i in range(B):
        for j in range(B):
            if i == j: continue
            if mask[i, j] > 0:
                loss += F.cosine_embedding_loss(proto_q[i], proto_s[j], label_matrix[i, j].unsqueeze(0), margin=margin)
                count += 1

    return loss / (count + 1e-6)
def info_nce_loss(proto_q, proto_s, class_ids, temperature=0.07, detach_query=True):
    """
    Compute InfoNCE contrastive loss between query and support prototypes.

    :param proto_q: [B, C] query prototype
    :param proto_s: [B, C] support prototype
    :param class_ids: [B] class IDs
    ...
    proto_q = F.normalize(proto_q, dim=1)  # [B, C]
    proto_s = F.normalize(proto_s, dim=1)
    # [B, C] × [C, B] → [B, B]
    logits = torch.mm(proto_q, proto_s.T) / temperature  # [B, B]
    labels = (class_ids.unsqueeze(1) == class_ids.unsqueeze(0)).float()  # [B, B]
    labels = labels / (labels.sum(dim=1, keepdim=True) + 1e-6)
    loss = - (labels * F.log_softmax(logits, dim=1)).sum(dim=1).mean()

    return loss
def classwise_proto_alignment_loss(proto_q, proto_s, class_ids):
    class_proto = {}
    for cls in torch.unique(class_ids):
        mask = (class_ids == cls)
        mean_proto_q = proto_q[mask].mean(dim=0)
        mean_proto_s = proto_s[mask].mean(dim=0)
        class_proto[cls.item()] = (mean_proto_q, mean_proto_s)

    loss = 0
    for cls, (q, s) in class_proto.items():
        loss += 1 - F.cosine_similarity(q.unsqueeze(0), s.unsqueeze(0))
    return loss / len(class_proto)