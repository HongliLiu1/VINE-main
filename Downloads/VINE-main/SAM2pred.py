...
class  SAM_pred(nn.Module):
    def __init__(self, ):
        super().__init__()
        self.sam_model = sam_model_registry['vit_h']('/mnt/disk18/liuhongli/FCP_v2/sam_vit_h_4b8939.pth')
        self.sam_model.eval()
    ...
    def forward_img_encoder(self, query_img):
        ...
        with torch.no_grad():
            ...
            image_encoder = self.sam_model.image_encoder
        return  query_feats
    ...
    def first_forward_mask_decoder(self, query_feats, q_sparse_em, q_dense_em, ori_size=(512, 512)):
        low_res_masks, iou_predictions = self.sam_model.mask_decoder(
            image_embeddings=query_feats,
            image_pe=self.sam_model.prompt_encoder.get_dense_pe(),
            sparse_prompt_embeddings=q_sparse_em,
